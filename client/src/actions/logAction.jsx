...
export const addLogs = (formData) => async dispatch => {
    ...
    try {
      ...
      const res = await axios.post(baseURL+'/api/logs', formData, config);
  ...
  export const deleteLogs = id => async (dispatch) => {
    setLoading();
    try {
        await axios.delete(baseURL+`/api/logs/${id}`);
        dispatch({
            type: DELETE_LOGS,
            payload: id
        })
...
export const updateLogs = (formData) => async dispatch => {
    ...
    try {
      const res = await axios.patch(baseURL+`/api/logs/${formData.id}`, formData, config);
      dispatch({
          type: UPDATE_LOG,
          payload: res.data
      })
...
export const searchLogs = (text) => async dispatch => {
    try {
      ...
      const res = await axios.get(baseURL+`/api/logs?q=${text}`);
