...
export const addLogs = (logs) => async (dispatch) => {
  ...
  try {
    ...
    const res = await axios.post("http://localhost:5050/logs", logs, config);
...
export const updateLogs = (log) => async (dispatch) => {
  ...
  try {
    const res = await axios.patch(
      `http://localhost:5050/logs/${log.id}`,
      log,
      config
    );
...
export const searchLogs = (text) => async (dispatch) => {
  try {
    ...
    const res = await axios.get(`http://localhost:5050/logs?q=${text}`);
